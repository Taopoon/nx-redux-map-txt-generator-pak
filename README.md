...
to generate `map.txt` files for FinalBurn Neo romsets, so the game list shows
real titles ("Metal Slug") instead of arcane rom names (`mslug.zip`).
...
- Rom folders tagged `(FBN)`, e.g. `Roms/Arcade (FBN)`
...
1. Pick the `(FBN)` rom folder.
2. Pick a dat file: a specific system (Arcade, Neogeo, …), *Use every Dat
   File*, or *Use local dat files* if you put any into `dats/`.
